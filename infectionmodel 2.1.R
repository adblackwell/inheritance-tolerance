...
runmodel<-function(params,ageburn=2000){
  with(params,{    
    ...
    infections<-data.frame(month=1,length=NA,R0=NA,RR=NA,immunekilled=NA,strategy=NA,hostdeath=NA,dieinfect=NA)
    ...
    for(i in 2:months){
      ...
      infections<-rbind(infections,data.frame(month=rep(i,sum(clearR==1 | clearT==1)),length=pop$infected[pop$infected>0][(clearR==1 | clearT==1)],R0=pop$R0[pop$infected>0][(clearR==1 | clearT==1)],RR=pop$RR[pop$infected>0][(clearR==1 | clearT==1)],immunekilled=clearR[(clearR==1 | clearT==1)],strategy=pop$strategy[pop$infected>0][(clearR==1 | clearT==1)],hostdeath=rep(0,sum(clearR==1 | clearT==1)),dieinfect=rep(0,sum(clearR==1 | clearT==1))))
      ...
      infections<-rbind(infections,data.frame(month=rep(i,nrow(addinfect)),length=addinfect$infected,R0=addinfect$R0,RR=addinfect$RR,immunekilled=rep(0,nrow(addinfect)),strategy=addinfect$strategy,hostdeath=rep(1,nrow(addinfect)),dieinfect=addinfect$dieinfect),make.row.names=FALSE)
    ...
    infections<-infections[!is.na(infections$length),]
