...
source("infectionmodel 3.0 For Publication.R")

#primary comparison?

params1<-makeparams(h2inf=0,h2past=0,Pim=0.05,infectedT0=0,Pmr=0,Pmt=0) #Base with no infection
params2<-makeparams(h2inf=0,h2past=0,Pim=0.05,noflip=TRUE) #No Tolerance
params3<-makeparams(h2inf=0,h2past=0,Pim=0.05) #Tolerance but no inheritance
params4<-makeparams(h2inf=1,h2past=0.1,Pim=0.05) #Tolerance and inheritance
...
strategyreport4(allpops[2:4],n=50*12,cols=cols,popmax=2000)
dev.off()

pdf("figures/survivalfig.pdf",width=3.43, height=3,pointsize=10)
par(mfrow=c(1,1),mar=c(5,4,1,1))
survplot2(allpops[1:4],n1=20*12+1,n2=120*12,cols=c("black",cols[3:5]),labs=c("No Infection","No Tolerance (A-B)","No Maternal Effect (C-D)","Maternal Effect (E-F)"))
dev.off()

## ---- modelflu ----
#set for flu. Medium cost immunity, same cost tolerance, high chance clearing, immunity
paramsflu<-makeparams(months=120,h2inf=1,h2past=0.1,R0=20,infectedT0=10,Pmr=0.10,Pmt=0.05,Pct=0,Pim=0.90)
