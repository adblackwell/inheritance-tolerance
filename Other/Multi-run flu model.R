...
paramsflu<-makeparams(months=120,h2inf=1,h2past=0.1,R0=20,infectedT0=10,Pmr=0.10,Pmt=0.05,Pct=0,Pcr=0.8,Pim=0.90)
...
seeds<-c(3,335,5478,6527,812,2367,12,563,4,90112,3546,13124)
popsfluM<-foreach(p=1:length(seeds)) %dopar% {
  set.seed(seeds[p])
  pops1<-runmodel(paramsflu,model=p)
  pops1
}
stopCluster(cl)
...
for(i in 1:length(popsfluM)){
  n<-12
  pops<-popsfluM[[i]]$pops[1:n]
  infections<-popsfluM[[i]]$infections
  params<-popsfluM[[i]]$params
