...
survplot2(allpops[1:3],n1=1,n2=100*12,cols=cols[3:5],labs=c("No Tolerance (A-B)","No Maternal Effect (C-D)","Maternal Effect (E-F)"))
dev.off()
...
survplot2<-function(allpops,n1=1,n2=length(allpops[[1]]$pops),cols=rainbow(length(allpops)),labs=seq(1:length(allpops))){
  ...
  plot(1,1,ylim=c(0,1),xlim=c(1,n2/12),type="n",ylab="Survival (%)",xlab=NA)
  ii<-1
  for(s in sss){
  ...
  mtext("Age",side=1,line=3)
  legend("topright",legend=labs,col=cols,lty=1,lwd=2,bty="o",box.col=NA,bg=rgb(1,1,1,0.5),inset=c(0.05,0.05))
}
